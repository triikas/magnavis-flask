...
def numbers():
    with open('numbers.json') as infile:
        ...
        if t - contracts_num[1] > a:
            contracts_num[0] = str(int(contracts_num[0])+((t - contracts_num[1]) // a))
            contracts_num[1] = t
        if t - client_num[1] > b:
            client_num[0] = str(int(client_num[0])+((t - client_num[1]) // a))
            client_num[1] = t
        if t - cargo_num[1] > c:
            cargo_num[0] = str(int(cargo_num[0])+((t - cargo_num[1]) // a))
            client_num[1] = t
